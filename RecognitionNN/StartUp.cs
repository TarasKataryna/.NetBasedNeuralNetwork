...
namespace RecognitionNN
{
    public class StartUp : IHostedService
    {
        ...
        public void Run(string filepath, bool onGpu, bool sgd, bool saveModel, CancellationToken cancellation)
        {
            ...
            try
            {
                var res = CreateCnnAndRun(filepath, onGpu, sgd, saveModel, true, cancellation);
                lossCollection = res.Item1;
                accuracyCollection = res.Item2;


                //var a = ((CNN)LastNetwork).Test(configuration["cifar10test"], 10000, 10, cancellation);
            }
            catch (Exception ex)
            {
            ...
            if (saveModel)
            {
                try
                {
                    ...
                    var id = SaveNetworkToDB(LastNetwork as CNN, "CIFAR-10-2");
                }
                catch (Exception ex)
                {
                    Console.WriteLine(ex.Message);
        ...
        private List<double> CreateNetworkAndRun(string filePath)
        {
           ...
           var outputLayerNeuronsCount = 10;
            ...
            var network = (MultilayerPerceptron)factory.CreateStandart(inputLayerNeuronsCount, hiddenLayerNeuronsCount, outputLayerNeuronsCount);
        ...
        private Tuple<List<double>, List<double>> CreateCnnAndRun(
            string imgInfoPath,
            bool onGpu,
            bool sgd,
            bool saveModel,
            ...
            CancellationToken cancellation)
        {
            ...
            //var inputLayerNeuronsCount = 768;
            //var hiddenLayerNeuronsCount = 150;
            ...
            var imagesPaths = configuration["cifar10"].Split(';');
            int dataCountInFile = 10000;

            var inputLayerNeuronsCount = 1280;
            ...
            {
                var res = network.SGDWholeFile(25,learningRate,64,lossEps,
                        imagesPaths,dataCountInFile,classCount,cancellation);
